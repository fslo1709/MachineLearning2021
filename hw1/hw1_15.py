...
for line in lines: #get x and y values
    ...
    size_vector = 0
    for i in range(n): #normalizing the vector with the size
        num = float(temp[i])
        size_vector += num**2
    size_vector = math.sqrt(size_vector) #getting the final size of the vector
    x_array = []
    x_array.append(1.0/size_vector) #scaled by the vector size
    for i in range(n):
        x_array.append(float(temp[i])/size_vector) #scaled by the vector size
